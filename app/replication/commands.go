...
func (repl *Replicator) RespondToWait(ctx context.Context, args []any) (*token.Token, error) {
	...
	defer repl.endBlock()
	res, err := repl.countAckFromFollowers(ctx, numReplicas, timeout)
	if err != nil {
		return nil, err
	}
...
func (repl *Replicator) countAckFromFollowers(ctx context.Context, numReplicas, timeoutSeconds int) (int, error) {
	...
	respond := func(port int, conn *net.TCPConn) {
		err := repl.getAckFromFollower(conn, timeoutSeconds, message)
		if err != nil {
			errs[port] = err
		}
	}
